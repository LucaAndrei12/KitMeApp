...
suspend fun addPost(token: String, base64image: String, title: String, scope: CoroutineScope): String{
...
suspend fun likePostAction(value: Boolean, postOID: String, token: String, scope: CoroutineScope): String{
    var wasSuccessful = "An error occurred."
...
suspend fun getFeed(token: String, scope: CoroutineScope): ArrayList<String>{
    var arr: ArrayList<String> = ArrayList<String>()
        ...
        if(getFeedResponse.body<String>().contains("wasSuccessful")){
            ...
            val toAddToArray = getFeedResponse.body<String>().split(",")
